        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
              <h2 className="text-white font-semibold">สร้างใบเบิก</h2>
              <button onClick={() => setReqModal(null)} className="text-slate-400 hover:text-white"><X size={18} /></button>
            </div>
            <div className="p-6 space-y-4">
              {/* Stock info */}
              <div className="bg-slate-800 rounded-lg p-3 space-y-1 text-sm">
                <p className="text-white font-medium">{reqModal.product?.part_name}</p>
                <p className="text-slate-400 text-xs font-mono">{reqModal.lot_no}</p>
                <p className="text-green-300 text-xs">มีในสต็อก {formatNumber(reqModal.qty)} {reqModal.unit}</p>
              </div>

              <div>
                <label className="block text-xs text-slate-400 mb-1.5">จำนวนที่เบิก ({reqModal.unit})</label>
                <input
                  type="number"
                  value={reqQty}
                ...
                />
                {parseFloat(reqQty) > reqModal.qty && (
                  <p className="text-red-400 text-xs mt-1">เกินยอดสต็อก ({formatNumber(reqModal.qty)} {reqModal.unit})</p>
                )}
              </div>

              <div>
                <label className="block text-xs text-slate-400 mb-1.5">เชื่อมกับ Sale Order</label>
                <select
                  value={soId}
                ...
                >
                  ...
                  {activeOrders.map(o => (
                    <option key={o.id} value={o.id}>
                      {o.so_no} — {o.customer?.name} ({formatNumber(o.qty)} {o.unit})
                    </option>
                  ))}
                </select>
              </div>

              {/* แสดง stock_qty ของ SO ที่เลือก */}
              {soId && (() => {
                const selSO    = orders?.find(o => o.id === soId)
                const stockQty = (selSO as any)?.stock_qty ?? 0
                if (stockQty <= 0) return null
                const stkLot  = goodStock.find(s => s.id !== reqModal.id && s.lot_no.startsWith(`STK-${selSO?.so_no}-`))
                const prodQty = parseFloat(reqQty) || reqModal.qty
                const stkAmt  = stkLot ? stkLot.qty : stockQty
                const total   = prodQty + stkAmt
                return (
                  <div className="bg-green-500/10 border border-green-500/25 rounded-lg px-4 py-3 text-xs space-y-1.5">
                    <p className="text-green-300 font-medium">SO นี้มีสต็อกเดิม — จะรวมในใบเบิกอัตโนมัติ</p>
                    <div className="flex justify-between text-slate-400">
                      <span>Lot นี้ (ผลิตมาใหม่)</span>
                      <span className="text-slate-300">{formatNumber(prodQty)} {reqModal.unit}</span>
                    </div>
                    <div className="flex justify-between text-green-400">
                      <span>สต็อกเดิม{stkLot ? ` (${stkLot.lot_no})` : ' ตาม SO'}</span>
                      <span className="text-green-300">+{formatNumber(stkAmt)} {selSO?.unit}</span>
                    </div>
                    {!stkLot && <p className="text-yellow-400 text-[10px]">⚠ ไม่พบ STK lot ในคลัง — จะใช้ยอดจาก SO แทน</p>}
                    <div className="flex justify-between font-semibold border-t border-slate-700 pt-1.5">
                      <span className="text-white">รวมใบเบิกทั้งหมด</span>
                      <span className="text-white">{formatNumber(total)} {reqModal.unit}</span>
                    </div>
                  </div>
                )
              })()}
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setReqModal(null)} className="flex-1 bg-slate-800 hover:bg-slate-700 text-white py-2 rounded-lg text-sm">ยกเลิก</button>
                <button
                  onClick={handleCreate}
                  disabled={!soId || createReq.isPending || parseFloat(reqQty) > reqModal.qty}
                  className="flex-1 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white py-2 rounded-lg text-sm transition-colors"
                >
                  {createReq.isPending ? 'กำลังสร้าง...' : 'สร้างใบเบิก'}
                </button>
              </div>
            </div>
          </div>
